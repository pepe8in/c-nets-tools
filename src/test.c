...
void *scanOpenPortsOnlyThread(void *arg) {
    ScanData *data = (ScanData *)arg;
    GString *results = g_string_new("");
...
void scanOpenPortsOnly(GtkWidget *button, gpointer data) {
    GtkWidget **widgets = (GtkWidget **)data;
    if (!widgets) return;
    ...
    g_thread_new("scan_open_ports_thread", scanOpenPortsOnlyThread, scan_data);
}
    ...
    for (GList *iter = children; iter != NULL; iter = iter->next) {
        if (GTK_IS_RADIO_BUTTON(iter->data) && gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(iter->data))) {
            ...
            if (g_strcmp0(label, "Scan d'un seul port") == 0) {
                ...
                data_array[2] = result_textview;
                data_array[3] = ip_entry; 
                g_signal_connect(scan_button, "clicked", G_CALLBACK(scanOpenPortsOnly), data_array);
